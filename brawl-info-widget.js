...
const brawlInfoWidget = {
  runScript: function(widgetParameter) {
      ...
      // Create Widget
      let mainWidget = await this.createWidget(playerTag);
  ...
  createWidget: async function(playerTag) {
    ...
    const appBG = await this.loadAppImg("https://i.imgur.com/TGoHbXq.jpg");
    const data = await this.getPlayerData(playerTag);
    ...
    let contentText1 = brawlerContentStack1.addImage(await this.createProgressBar(data.brawlerRanks.rank35s, data.brawlers.length, new Color("8404b3")));
    ...
    let contentText2 = brawlerContentStack2.addImage(await this.createProgressBar(data.brawlerRanks.rank30s, data.brawlers.length, new Color("f01818")));
    ...
    let contentText3 = brawlerContentStack3.addImage(await this.createProgressBar(data.brawlerRanks.rank25s, data.brawlers.length, new Color("00ff8c")));
    ...
    let contentText4 = brawlerContentStack4.addImage(await this.createProgressBar(data.brawlerRanks.rank20s, data.brawlers.length, new Color("e004bc")));
